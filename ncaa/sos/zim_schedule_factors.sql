...
drop table if exists ncaa._zim_schedule_factors;

create table ncaa._zim_schedule_factors (
        school_id			integer,
	year				integer,
	year_factor			float,
        park                    	float,
...
insert into ncaa._zim_schedule_factors
...
from ncaa._zim_factors o
--left outer join ncaa._zim_factors d
join ncaa._zim_factors d
  on (d.level,d.year,d.parameter)=(o.level,o.year,'defense')
--left outer join ncaa._zim_factors p
join ncaa._zim_factors p
  on (p.level,p.parameter)=(o.level,'park')
--left outer join ncaa._zim_factors y
join ncaa._zim_factors y
  on (y.level::integer,y.parameter)=(o.year,'year')
where o.parameter='offense'
);

update ncaa._zim_schedule_factors
...
--park=o.park,
strength=o.strength
from ncaa._zim_schedule_factors o
...
update public.r
set park=p.park
from ncaa._zim_schedule_factors p
...
from rs
where
  (_zim_schedule_factors.school_id,_zim_schedule_factors.year)=
